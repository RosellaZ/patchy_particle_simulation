...
SIM_DIR = ROOT_DIR + f'sc2_N{N}_nsteps{n_steps}_kT{kT}_am{alpha_morse}_stronge{strong_e}_ts{threshold}_bs{ensemble_size}/'
p = Path(SIM_DIR)
if not p.exists():
  os.mkdir(SIM_DIR)
...
def run_simulation(key, num_steps, init_pos, init_species, threshold = 0.202, strong_e = 20.0, weak_e = 4.0, kT = 1.0, alpha_morse = 9.0, alpha_soft = 2.0, r_cutoff = 1.0):
  ...
  morse_eps = jnp.vstack((jnp.hstack((gen_morse_eps(energy_patch0), gen_morse_eps(energy_patch0))), jnp.hstack((gen_morse_eps(energy_patch0), gen_morse_eps(energy_patch1)))))
