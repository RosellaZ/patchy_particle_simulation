...
OPT_DIR_NAME = '../Simulation_Results/' + f'state_change_N{N}_kT{kT}_nsteps{n_steps}'
p = Path(OPT_DIR_NAME)
if not p.exists():
...
key = random.PRNGKey(2234)
key, split = random.split(key)
start = time.time()
st, sp, posss, spsss, energies = run_simulation(split, 40000, init_pos, species0, energy_patch0, energy_patch1)
end = time.time()
duration = end - start
print(f"Simulation steps {n_steps} for {N} particles, takes {duration} seconds in total")

pickle.dump(st, open(OPT_DIR_NAME+'/final_state', 'wb'))
pickle.dump(sp, open(OPT_DIR_NAME+'/final_species', 'wb'))
pickle.dump(posss, open(OPT_DIR_NAME+'/state_traj', 'wb'))
pickle.dump(spsss, open(OPT_DIR_NAME+'/species_traj', 'wb'))
pickle.dump(energies, open(OPT_DIR_NAME+'/energy_traj', 'wb'))