...
REF_SHAPE = jnp.array([[0.0, 0.0], 
                      [0.0, 2*center_particle_rad],
                      [2*center_particle_rad, 0.0], 
                      [2*center_particle_rad, 2*center_particle_rad]])
...
def sys_loss(R):
  ...
  substract = lambda R, R_REF: R - R_REF
  v_substract = space.map_product(substract)
  diffs = v_substract(ds[:, :len(REF_SHAPE)], ref_dists)
  loss_fun = jnp.sum(jnp.mean(jnp.linalg.norm(diffs, axis = -1), axis = 0))
  return loss_fun
...
def avg_loss(R_batched):
  losses = vmap(sys_loss)(R_batched)
...
def get_mean_loss(theta, sim_keys, num_steps_opt, init_poss):
  ...
  mean_loss = avg_loss(states.position.center)
...
OPT_DIR_NAME = '../Simulation_Results/' + 'square_Opt_kT{}_nparticle{}_nsteps{}_noptsteps{}_nopt{}_batchsize{}_theta{}'.format(kT, N, n_steps, n_steps_opt, opt_steps, ensemble_size, params)
