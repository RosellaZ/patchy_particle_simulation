...
DIR_NAME = '../Simulation_Results/' + 'opted' + '_kT{}_nparticle{}_nsteps{}_batchsize{}_{}'.format(kT, N, n_steps, ensemble_size, int(sys.argv[1]))
p = Path(DIR_NAME)
if not p.exists():
  os.mkdir(DIR_NAME)
...
previous_loss = onp.loadtxt(OPT_DIR_NAME + '/loss' + str(0.005) + '.txt')
params = previous_sim_param[onp.argmin(previous_loss)]

init_poss = gen_init_pos(N, box_size, random.split(split, ensemble_size))

key, split = random.split(key)

start = time.time()
state_afters, lossss, energies = v_sim(params, patch_energies, random.split(split, ensemble_size), n_steps, init_poss, max_energy_center, kT, r_cutoff)
end = time.time()
duration = end - start
print(f"Ensemble size = {ensemble_size}, {duration} seconds in total")
